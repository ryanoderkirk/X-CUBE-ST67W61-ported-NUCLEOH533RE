...
if "%BOARD_ID%" == "NUCLEO-H533RE" (
    echo masquerading H33 as H63...
    set BOARD_ID=NUCLEO-H563ZI
)
...
%current_dir%QConn_Flash\QConn_Flash_Cmd.exe --port %COMADDR% --config %current_dir%NCP_Binaries\mission_t01_flash_prog_cfg.ini --efuse=%current_dir%\NCP_Binaries\efusedata.bin
if %ERRORLEVEL% neq 0 goto :error
echo NCP Flashing in DONE!!
