...
QConn_Flash/QConn_Flash_Cmd-ubuntu --port $COMADDR --config "$current_dir/NCP_Binaries/mission_t01_flash_prog_cfg.ini" --efuse="$current_dir/NCP_Binaries/efusedata.bin"
if [ $? -ne 0 ]; then
  echo -e "${RED}!!!! Error detected !!!!${NC}"
  exit 1
fi
...
if [ "$BOARD_ID" = "NUCLEO-N657X0-Q" ]; then
    ...
    echo "Press any key to continue..."
    read -r junk
    STM32_Programmer_CLI -c port=swd mode=ur -w "$current_dir/NUCLEO-N657X0-Q_Binaries/ST67W6X_CLI_FSBL.bin" 0x70000000 --extload "$CUBEPROGRAMMER/ExternalLoader/MX25UM51245G_STM32N6570-NUCLEO.stldr"
    if [ $? -ne 0 ]; then
      echo -e "${RED}!!!! Error detected !!!!${NC}"
      exit 1
    fi
    ...
    STM32_Programmer_CLI -c port=swd mode=ur -w "$current_dir/"$BOARD_ID"_Binaries/ST67W6X_CLI_Boot.bin" 0x08000000
    if [ $? -ne 0 ]; then
      echo -e "${RED}!!!! Error detected !!!!${NC}"
      exit 1
    fi
    STM32_Programmer_CLI -c port=swd mode=ur -w "$current_dir/"$BOARD_ID"_Binaries/ST67W6X_CLI_Appli.bin" 0x70000000 --extload "$CUBEPROGRAMMER/ExternalLoader/MX25UW25645G_NUCLEO-H7S3L8.stldr"
    if [ $? -ne 0 ]; then
      echo -e "${RED}!!!! Error detected !!!!${NC}"
      exit 1
    fi
else
    STM32_Programmer_CLI -c port=swd mode=ur -w "$current_dir/"$BOARD_ID"_Binaries/ST67W6X_CLI.bin" 0x08000000 --go
    if [ $? -ne 0 ]; then
      echo -e "${RED}!!!! Error detected !!!!${NC}"
      exit 1
    fi
